/**
 * board_pins_c3.h —— ESP32-C3 引脚表（2 通道降级方案）
 * ============================================================================
 *
 * ESP32-C3 总共只有 10 个通用 GPIO（0-9），其中：
 *   - GPIO4/5 = SPI Flash 占用（模组内部），不可用
 *   - GPIO8/9 = strapping（BOOT 按钮 / 启动模式），仅作输入
 *   - 实际可用 6 个脚（0,1,2,3,6,7）
 *
 * 分配方案（电机 2 + 离合 4 = 6 个输出脚占满）：
 *   - 电磁离合 ×4：GPIO0/1/2/3（全部输入，微动开关对 GND + 内部上拉）
 *   - 电机 H 桥 ×2：GPIO6/7（PWM 输出）
 *   - 微动开关：无可用脚，全部走 MQTT 事件或超时模式
 *   - 挤出机到位：走 MQTT 事件（hw_switch_state）
 *
 * 与 S3 版的区别：
 *   - BOARD_CHANNEL_COUNT = 2（只有 2 路离合）
 *   - 所有传感器引脚 = -1（不可用，降级为超时模式）
 *   - BOARD_EXTRUDER_INPLACE_UNUSED = true（完全依赖 MQTT）
 *   - BOARD_GPIO_MAX = 10
 *   - C3 的保留脚是 GPIO12-17（SPI Flash）和 GPIO20/21（UART）
 */
...
#define BOARD_PIN_CLUTCH {0, 1}   /* 电磁离合 1~2（仅 2 路） */
#define BOARD_PIN_LED -1          /* C3 DevKit 板载 LED 是 WS2812，用 GPIO8 但它是 strapping，这里不用 */

/* ==========================================================================
 * 三、★ 微动开关（输入）：C3 无可用脚，全部 -1 ★
 * ==========================================================================
 * -1 表示该传感器不可用，filament_sensor.c 会跳过对应 GPIO，
 * 进料流程退化为纯超时模式（AMS_NO_LIMIT_LOAD_MS 计时封顶）。
 */
/** 停止送料微动：C3 无脚，退化为超时 */
#define BOARD_PIN_SENSOR_STOP     {-1, -1}
/** 开始送料微动：C3 无脚 */
#define BOARD_PIN_SENSOR_START    {-1, -1}
/** 自吸微动：C3 无脚 */
#define BOARD_PIN_SENSOR_AUTOLOAD {-1, -1}
...
/** C3 的 strapping 脚：GPIO2/8/9 */
#define BOARD_STRAPPING_PINS {2, 8, 9}
